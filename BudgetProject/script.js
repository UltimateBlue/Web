let UIController = (function () {
  ...
  let addIncRecord = function (newRecord) {
    ...
    document
      .getElementById(DOMStrings.incList)
      .insertAdjacentHTML("afterbegin", newHtml);
  };

  let addExpRecord = function (newRecord) {
    let html =
      '<div id="exps-%id%" class="list-items"><div class="exp-des"><p>%descript%</p></div><div class="exp-val">   <p>%value%</p><div class="exp-percent">%perc%%</div><svg class="del-item" id="del-exp-item%id% width="32px" height="32px" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><path d="M 16 3 C 8.832031 3 3 8.832031 3 16 C 3 23.167969 8.832031 29 16 29 C 23.167969 29 29 23.167969 29 16 C 29 8.832031 23.167969 3 16 3 Z M 16 5 C 22.085938 5 27 9.914063 27 16 C 27 22.085938 22.085938 27 16 27 C 9.914063 27 5 22.085938 5 16 C 5 9.914063 9.914063 5 16 5 Z M 12.21875 10.78125 L 10.78125 12.21875 L 14.5625 16 L 10.78125 19.78125 L 12.21875 21.21875 L 16 17.4375 L 19.78125 21.21875 L 21.21875 19.78125 L 17.4375 16 L 21.21875 12.21875 L 19.78125 10.78125 L 16 14.5625 Z"/></svg></div></div>';
    ...
    document
      .getElementById(DOMStrings.expList)
      .insertAdjacentHTML("afterbegin", newHtml);
  };

  let updateHeader = function (newRecord) {
    document.getElementById(DOMStrings.currentAmount).textContent =
      formatNumbers(newRecord[1], "inc");
    document.getElementById(DOMStrings.currentIncome).textContent =
      formatNumbers(newRecord[2], "inc");
    document.getElementById(DOMStrings.currentExpense).textContent =
      formatNumbers(newRecord[3], "exp");
    document.getElementById(DOMStrings.expenseTotalPerc).textContent =
      newRecord[4] + "%";
  };
  ...
  return {
    ...
    clearFields: function () {
      ...
      document.querySelector(DOMStrings.descript).value = "";
      document.querySelector(DOMStrings.moneyAmount).value = "";
      document.querySelector(DOMStrings.descript).focus();
    ...
    changeColor: function () {
      document.querySelector(DOMStrings.descript).classList.toggle("redborders");
      document.querySelector(DOMStrings.moneyAmount).classList.toggle("redborders");
      document.querySelector(DOMStrings.btn).classList.toggle("btn-red");
    },
    setDate: function () {
      let today, year, month, monthNames, txt, newtxt;
      today = new Date();
      year = today.getFullYear();
      month = today.getMonth();

      monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
      txt = 'Available Budget in %month% %year%:'
      newtxt = txt.replace('%month%', monthNames[month]);
      newtxt = newtxt.replace('%year%', year);
      document.querySelector('.current-date').textContent = newtxt;
...
let Controller = (function (dataCTRL, uiCTRL) {
  ...
  let getInput = function () {
    // GET UI FIELDS DATA
    let descript = document.querySelector(DOMStrings.descript).value;
    let val = parseFloat(document.querySelector(DOMStrings.moneyAmount).value);
  ...
  let init = (function () {
    // ADD EVENT HANDLERS
    document
      .getElementById(DOMStrings.comboValue)
      .addEventListener("change", uiCTRL.changeColor);
    document
      .getElementById(DOMStrings.footerLists)
